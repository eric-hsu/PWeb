...
public class ParamChecking {
	...
	public static boolean isNum(String param) {
		...
		if (param.indexOf("-") != param.lastIndexOf("-")
			...
			return false;
		if (param.length() == 1
				&& (param.substring(0).equalsIgnoreCase("-") || param
	 ...
	 * 
	 * @author 
	 * @Title: specialStrEscape
	 * @Description: 对查询参数中特殊字符转义处理
	 * @param param
	 * @return String
	 */
	public static String specialStrEscape(String param) {
		if(StringUtils.isNotBlank(param)) {
			param = param.replace("/", "//").replace("'", "''")
						 .replace("%", "/%").replace("％", "/％")
						 .replace("_", "/_").replace("＿", "/＿");
		}
		return param;
	}
...
//		System.out.println(isPositiveInteger("-123123312323"));
//	}

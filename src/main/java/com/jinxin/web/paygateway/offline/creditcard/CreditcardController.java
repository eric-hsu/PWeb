package com.jinxin.web.paygateway.offline.creditcard;
 ...
 */
@Controller
@RequestMapping(value = "/creditcardController")
@Scope("prototype")
public class CreditcardController extends BaseContronller{
	private static final Logger logger = Logger.getLogger(CreditcardController.class);
	...
	public String quickPay(@ModelAttribute("card")Card card,HttpServletRequest request, HttpServletResponse response,Model model,RedirectAttributes redirectAttributes){
		...
		//3，相应service接口配置获取
		String operatemode = StaticMapUtil.getOfflineCreditcardServiceMap().get("opm"+card.getBankCode());
		if(operatemode==null){
